...
状態: 2026-09-16 の設計に基づく実装済み。2026-09-18 のレビュー修正と D24〜D29 を反映。設計の正典はこのファイル。
...
同日に D24〜D28 を実装。2026-09-19 に D29 を実装。bash / zsh の関数は実バイナリで `awsp <profile> --no-login` の反映まで確認した。fish は作者の環境に無く、構文の確認は未実施。
